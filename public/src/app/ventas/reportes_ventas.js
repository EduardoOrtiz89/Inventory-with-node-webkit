angular.module('ngBoilerplate.reportes_ventas', [
  'ui.router',
  'ngResource',
  'util.datepicker'
])
  .config(function config($stateProvider) {
    $stateProvider.state('reportes_ventas', {
      url: '/ventas-reportes',
      views: {
        "main": {
          controller: 'reportesVentasCtrl',
          templateUrl: 'ventas/reportes_ventas.tpl.html'
        }
      },
      data: {
  ...
  .controller('reportesVentasCtrl', function reportesVentasCtrl($scope, $templateCache,$compile,TicketsVentas, DatePicker, SettingsGet, Prendas, ReportesVentas, Dates, $location, ReportesCache) {
    var type = "reportes";
    SettingsGet.get({
...
var prendasCodigo = ["sacos", "pantalones", "chalecos"];
  ...
  var prendasCantidad = ["togas", "corbatas", "corbatines", "gaznes", "monios", "zapatos"];
    ...
    $scope.getTicket = function(item) {
      $location.path('/tickets_ventas/' + item.id);
    };
