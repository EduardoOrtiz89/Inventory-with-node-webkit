...
.controller('modalController', function($scope, $compile, $filter, $modalInstance, tables, FormFactory, TableSearch, prenda, articulos,$stateParams) {
  ...
  tables[prenda.name].get({ funcion: {OR: ["2","3"]} ,borrado:0},function(data) {
    $scope.items = data;
    $scope.search();
  ...
  $scope.add = function(item) {
    ...
    item.subtotal = parseFloat($scope.cantidad) * parseFloat(item.costo_venta);
...
.controller('ventasCtrl', function ventasController($scope, NumTicket, $dialogs, $window, $templateCache, $compile, $cookies, $log, $filter, $modal, apartados,
  $location, tables, Prendas, Tickets,$stateParams,SettingsGet) {
